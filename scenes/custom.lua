...
scene.widgetList={
    ...
    WIDGET.new{type='slider',x=250,y=380,w=500,axis={1,5,1},labelDistance=40,textAlwaysShow=true,disp=function() return model end,valueShow=function(s) return Options.name.model[s._pos0] end,code=function(i) model=i end},
    WIDGET.new{type='button_fill',x=150,y=500,w=80,fontSize=25,color='lB',text="Code",code=function()
        ...
        if not pcall(PlayFromCode,code) then
            MSG.new('error',"Invalid riddle code: "..(code and #code>0 and code or "?"))
        end
    end},
